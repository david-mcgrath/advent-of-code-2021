...
await getInput(true)
	.then(input => {
			...
			return scanners
				.map(s => {
					return {
						id: s.id,
						scanner: s.scanner,
						scannerPositions: s.scannerPositions.map(x => { return { x: x.x, y: x.y, z: x.z }; }),
						beacons: s.beacons,
		...
		while (scanners.length > 1) {
			let pairs = scanners
				...
				.map(pair => {
					...
					// I got lazy. Better performance isn't required at all for p2, and this day has been rough enough that I just want to get a result.
			...
			for (let i = 0; i < pairs.length; i++) {
				...
				// This could be modified to improve performance, it modifying it to allow merging more than 2 scanners in one step.
				// It will still give the right answer without it though.
				...
				for (let j = 0; j < orientations.length; j++) {
					...
					if (!failed) {
						...
						let merged = {
							...
							scannerPositions: pair.pair0.scannerPositions.concat(pair.pair1.scannerPositions.map(position)),
			...
			// This was being hit when there were bugs, but now it appears to be fine
		...
		let maxDistance = 0;
		let positions = scanners.flatMap(x => x.scannerPositions);
		for (let i = 0; i < positions.length; i++) {
			for (let j = 0; j < positions.length; j++) {
				let dist = Math.abs(positions[i].x - positions[j].x) + Math.abs(positions[i].y - positions[j].y) + Math.abs(positions[i].z - positions[j].z);
				if (dist > maxDistance) {
					maxDistance = dist;
				}
			}
