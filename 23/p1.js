...
class MoveOption {
	...
	CheckValid(map) {
		...
		if (this.RoomIndex === 1 && map.Rooms[this.RoomNumber][0])
			return false;
		...
		if (this.Type === MoveTypes.HallwayToRoom) {
			...
			if (amphipod.DestinationRoom !== this.RoomNumber)
				return false;
			if (map.Rooms[this.RoomNumber][1 - this.RoomIndex] && map.Rooms[this.RoomNumber][1 - this.RoomIndex].DestinationRoom !== this.RoomNumber)
				return false;
		}
		...
		if (this.Type === MoveTypes.RoomToHallway) {
			if (map.Rooms[this.RoomNumber][this.RoomIndex].DestinationRoom === this.RoomNumber && map.Rooms[this.RoomNumber][1 - this.RoomIndex] && map.Rooms[this.RoomNumber][1 - this.RoomIndex].DestinationRoom === this.RoomNumber) {
				return false;
			}
		}
...
class Map {
	constructor() {	
		...
		this.Rooms = [[null,null],[null,null],[null,null],[null,null]];
		this.CurrentCost = 0;
	}
...
await getInput(false)
	.then(input => {
		...
		baseMap.Rooms[0][1] = new Amphipod(input[3][3]);
		baseMap.Rooms[1][0] = new Amphipod(input[2][5]);
		baseMap.Rooms[1][1] = new Amphipod(input[3][5]);
		baseMap.Rooms[2][0] = new Amphipod(input[2][7]);
		baseMap.Rooms[2][1] = new Amphipod(input[3][7]);
		baseMap.Rooms[3][0] = new Amphipod(input[2][9]);
		baseMap.Rooms[3][1] = new Amphipod(input[3][9]);
