...
await getInput(false)
    .then(input => {
        ...
        for (var i = 0; i < numbers.length && boards.length > 0; i++) {
                ...
                for (var k = 0; k < 5; k++) {
                    for (var l = 0; l < 5; l++) {
                        if (boards[j].board[k][l] === currentNumber) {
                            ...
                            if (boards[j].maxScore < boards[j].rows[k]) {
                                boards[j].maxScore = boards[j].rows[k];
                            }
                            if (boards[j].maxScore < boards[j].columns[l]) {
                                boards[j].maxScore = boards[j].columns[l];
                            }
                        }
                    }
                }
            ...
            boards = boards.filter(x => x.maxScore < 5);
        }
